...
async function shareVideo() { if (!lastVideo) await exportVideo(false); if (!lastVideo) return; const file = new File([lastVideo], `${fileName()}.webm`, { type: 'video/webm' }); if (navigator.share && (!navigator.canShare || navigator.canShare({ files: [file] }))) { try { await navigator.share({ title: $('#projectName').value, text: 'Un souvenir à partager', files: [file] }); return; } catch (error) { if (error.name === 'AbortError') return; } } download(lastVideo, `${fileName()}.webm`); window.open(`https://wa.me/?text=${encodeURIComponent(`Voici mon souvenir « ${$('#projectName').value} ». La vidéo vient d'être téléchargée.`)}`, '_blank'); showToast('Vidéo téléchargée, puis WhatsApp est ouvert.'); }
